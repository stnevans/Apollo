...
const U64 whiteKingSideCastling = 0x31d71dce64b2c310L;
const U64 whiteQueenSideCastling = 0xf165b587df898190L;
const U64 blackKingSideCastling = 0xa57e6339dd2cf3a0L;
const U64 blackQueenSideCastling = 0x1ef6e6dbb1961ec9L;
const U64 passantColumn[] =
...
const U64 whiteMove = 0xf8d626aaaf278509L;
...
U64 getKeyForMove(bool pieceMovedWhite, U8 fromLoc, U8 toLoc, PieceType piece){
	return getSquareKey(pieceMovedWhite, fromLoc, piece) ^ getSquareKey(pieceMovedWhite, toLoc, piece);
}
...
U64 initKeyFromBoard(Board * b){
	...
	U64 key = 0;
	for(int i = 0; i < 64; i++){
		U64 square = getSquare[i];
		if((b->currentBoard()->AllPiecesBB & getSquare[i]) != 0){
			bool isWhite = (b->currentBoard()->WhitePiecesBB & getSquare[i]) != 0;
			PieceType type;
			if(isWhite){
			...
			key ^= getSquareKey(isWhite, i,type);
		}
	}
	if(b->currentBoard()->whiteKingCastle){
			key ^= whiteKingSideCastling;
	}
	if(b->currentBoard()->whiteQueenCastle){
		key ^= whiteQueenSideCastling;
	}
	if(b->currentBoard()->blackKingCastle){
		key ^= blackKingSideCastling;
	}
	if(b->currentBoard()->blackQueenCastle){
		key ^= blackQueenSideCastling;
	}
	if(b->currentBoard()->enPassantLoc != ENPASSANT_NONE){
			key ^= passantColumn[getFile(b->currentBoard()->enPassantLoc)];
	}
